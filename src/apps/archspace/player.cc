...
CPlayer::build_control_model()
{
	...
	if (bPlayerHasPAAR)
		mControlModel.change_genius(1);
	if (bPlayerHasPAMS)
		mControlModel.change_military(1);
	if (bPlayerHasPACA)
		mControlModel.change_production(1);
	if (bPlayerHasPAATL)
	{
		mControlModel.change_facility_cost(1);
		mControlModel.change_research(5);
		mControlModel.change_military(1);
	}
...
int
CPlayer::get_research_cost(CTech *aTech)
{
	int ResearchCost = 20000 * (int)pow((double)2.0, (double)(aTech->get_level()));
	...
	if (RealResearchCost > MAX_RESEARCH || RealResearchCost < 0) return (int)MAX_RESEARCH;
...
//	mBountyList.expire_bounties();
	...
	if (get_research_invest() > 0)
	{
		...
		{
			Invest = (long long int)get_research_lab()*10 * 2;
		}
		else
		{
			Invest = (long long int)get_research_lab()*10;
		}
	...
	for (int i=0 ; i<mPlanetList.length() ; i++)
	{
		...
		if (Planet->update_destroy_timer() == true)
		{
			...
			Upkeep += (long long int)Planet->get_upkeep_per_turn();
	...
	for (int i=0 ; i<FORTRESS_LIST->length() ; i++)
	{
		CFortress *
			Fortress = (CFortress *)FORTRESS_LIST->get(i);
		if (Fortress->get_owner_id() == mGameID) MPFromFortress += 10000;
	}
...
/*	if  ((get_race() ==1) && (has_project(9001)))
	...
	else if (is_protected() && time(0) - mLastLostPlanet->time >= 60*60*12-CGame::mSecondPerTurn)
	{
	...
	for (int i=0 ; i<mFleetList.length() ; i++)
	{
		...
		for (int i=0 ; i<DEVICE_MAX_NUMBER ; i++)
		{
			if (Fleet->get_device(i) == SPACE_MINING_MODULE && Fleet->get_status() != CFleet::FLEET_DEACTIVATED)
			{
				...
				int
					PPDiff = Fleet->get_size() * (100 + NumberOfTech) * Fleet->get_current_ship();
...
CPlayer::repair_damaged_ship()
{
	for (int i=mRepairBay.length()-1 ; i>=0 ; i--)
	{
		...
		int
			Upkeep = (int)UpkeepTable[Ship->get_body()-4001];
...
CPlayer::pay_fleet_upkeep(int aNetIncome)
{
	...
	for (int i=0 ; i<mDock.length() ; i++)
	{
		...
		int
			ShipUpkeep = (int)(Ship->get_number()*UpkeepTable[Ship->get_body()-4001]/10);
		if (MAX_PLAYER_MP - Upkeep < ShipUpkeep)
		{
			Upkeep = MAX_PLAYER_MP;
			break;
		}
	...
	if (Upkeep > mNewResource.get(RESOURCE_MILITARY))
	{
		...
		if ((long long int)aNetIncome + (long long int)mProduction < (long long int)Upkeep)
		{
	//	find a stand-by fleet and deactive it
	//	if every stand-by fleet is deactivated, scrap 10% of docked ship
			CFleetList
				CandidateFleetList;
			for (int i=0 ; i<mFleetList.length() ; i++)
			{
...
CPlayer::calc_all_ships_upkeep(int *aReservedPP, int aCurrentProductionPoint, int *aNewMilitaryPoint, int *aGainingProductionPoint, int *aLosingProductionPoint, int *aLosingMilitaryPoint)
{
	...
	for (int i=0 ; i<mDock.length() ; i++)
	{
		...
		Upkeep += (int)(Ship->get_number()*UpkeepTable[Ship->get_body()-4001]/10);
	}
...
{
	int nEffectiveMilitaryCM = mControlModel.get_military();
	if (nEffectiveMilitaryCM > 10)
	{
		nEffectiveMilitaryCM = 10; //Cap mil CM for ship prod.
	}
	int
		ret = (int)( (long long int) ((long long int)(mLastTurnResource.get(RESOURCE_PRODUCTION))*(30+(long long int)(nEffectiveMilitaryCM)*5)) /100 );
	if( ret < 0 ) return 0;
	return ret;
}
...
/*
	player.cc -> council �� ���涧 speaker �� ����
	db: CouncilForum

	catagories �� COUNCIL_ID �Է�ed
	forums �� COUNCIL_ID �Է�ed
	forum_mods �� speaker �� ����� �ֱ�ed (user_id = mGameID)
	users �� speaker �ֱ�ed
*/
		...
		if (Mission.is_over())
		{
			switch(Mission.get_mission())
			{
				case CMission::MISSION_TRAIN :
				{
					...
					{
						if (AdmiralExp < 100 + 500* Admiral->get_level()) AdmiralExp = 100 + 500* Admiral->get_level();
...
CPlayer::war_target_list_html()
{
	...
	for (int i=0 ; i<mRelationList.length() ; i++)
	{
		...
		List.format("<TD WIDTH=\"70\" CLASS=\"tabletxt\" ALIGN=\"CENTER\">%s</TD>\n",
...
CPlayer::war_status_information_html()
{
	...
	for (int i=0 ; i<mRelationList.length() ; i++)
	{
		...
		Info.format("<TD CLASS=\"tabletxt\" WIDTH=\"52\" ALIGN=\"CENTER\">%s</TD>\n",
...
CPlayer::privateer(CFleet *aFleet)
{
	...
	else
	{
		...
		if (!Planet->get_privateer())
		{
			...
			if (Admiral->get_level() >= 15 && Admiral->get_racial_ability() == CAdmiral::RA_FAMOUS_PRIVATEER)
                		PrivStealBonus = 1.0;
            else
            {
                PrivStealBonus = Admiral->get_level()/20.0;
                if (PrivStealBonus < -1.0)
                   PrivStealBonus = -1.0;
                else if (PrivStealBonus > 0.9)
                   PrivStealBonus = 0.9;
                for (int i=21; i < Admiral->get_level() && PrivStealBonus <= 1.0; i++)
                   PrivStealBonus += 0.02;
			}
...
//		SLOG("REAL EARN AMOUNT:%d", AmountEarned);
		...
		Exp_Gain = (int)(AmountEarned*Exp_Gain_Percent);
...
//							dec2unit(-DiffProduction));
		...
		case CPlayerEffect::PA_GAIN_SECRET_PROJECT :
		{
			/*CProject *
				Project = PROJECT_TABLE->get_by_id(aEffect->get_target());*/
			#define NUMBER_OF_SECRETBM_PROJECTS		4
