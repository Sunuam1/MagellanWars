...
CCouncilTable::compare_key(TSomething aItem, TConstSomething aKey) const
{
	...
	if (Council->get_id() > (intptr_t)aKey) return 1;
	if (Council->get_id() < (intptr_t)aKey) return -1;
	return 0;
}
...
/*	CCouncilActionMergingOffer
	...
	for (int i=AdmissionList->length()-1 ; i>=0 ; i--)
	{
		...
		Owner->time_news(format(GETTEXT("Your admission request has been canceled because the council %1$s disappeared."), Council->get_nick()));
