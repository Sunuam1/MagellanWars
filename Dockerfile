...
stderr_logfile=/dev/stderr
stderr_logfile_maxbytes=0
environment=DB_HOST="%(ENV_DB_HOST)s",DB_NAME="%(ENV_DB_NAME)s",DB_USER="%(ENV_DB_USER)s",DB_PASSWORD="%(ENV_DB_PASSWORD)s"
SUPERVISOR
...
for i in {1..30}; do
    if mysql -h${DB_HOST} -u${DB_USER} -p${DB_PASSWORD} ${DB_NAME} -e "SELECT 1" &>/dev/null; then
        echo "Database ready!"
        mysql -h${DB_HOST} -u${DB_USER} -p${DB_PASSWORD} ${DB_NAME} -e "UPDATE player SET tick = UNIX_TIMESTAMP() + 300 WHERE game_id > 0 AND (tick = 0 OR tick IS NULL)"
        break
    fi
    sleep 2
done
