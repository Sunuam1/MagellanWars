	...
	CPlanet::get_population_with_unit()
{
	if (mPopulation < 1000)
	{
		{ static CString tmp; tmp.format("%s k", dec2unit(mPopulation)); return (char *)tmp; }
	}
	else
	{
		{ static CString tmp; tmp.format("%s M", dec2unit(mPopulation/1000)); return (char *)tmp; }
	}
}
	...
	CPlanet::get_max_population_with_unit()
{
	...
	{
		{ static CString tmp; tmp.format("%s k", dec2unit(mMaxPopulation)); return (char *)tmp; }
	} else
	{
		{ static CString tmp; tmp.format("%s M", dec2unit(mMaxPopulation/1000)); return (char *)tmp; }
	}
}
	...
	if (get_blockade())
	{
		if (mBlockadeTimer < NowGameTime)
		{
			...
			ADMIN_TOOL->add_blockade_log(
				format("The blockade has expired on the planet %s whose owner is %s.",
	...
	CPlanet::process_privateer(time_t aNewGameTime)
{
	...
	if (mPrivateerTimer < aNewGameTime)
	{
		...
		ADMIN_TOOL->add_privateer_log(
			format("The planet %s, whose owner is the player %s, isn't under privateer anymore.",
				(char *)mName, get_owner()->get_nick()));
	}
}
...
//		SLOG("Temperature is going down %d", mTemperature);
		get_owner()->time_news(
			format(GETTEXT("Terraforming : The temperature has gone down to %1$s K on %2$s."),
	...
	CPlanet::get_order_name()
{
	switch(mOrder)
	{
		...
		default:
			{ static CString tmp; tmp.format(GETTEXT("The %1$sth Planet"),
				dec2unit(mOrder+1));
	}
}
