...
CBidList::compare_key( TSomething aItem, TConstSomething aKey ) const
{
	CBid
		*bid = (CBid*)aItem;
	if ( bid->get_id() > (intptr_t)aKey ) return 1;
	if ( bid->get_id() < (intptr_t)aKey ) return -1;
	return 0;
}
...
CBlackMarket::create_new_tech()
{
	...
	ADMIN_TOOL->add_new_bid_log(
			format("New tech. %s has been created.", tech->get_name_with_level()));
}
...
#undef MIND_TRACKER
	...
	ADMIN_TOOL->add_new_bid_log(
			format("New fleet %s has been created.", fleet->get_nick()));
...
CBlackMarket::create_new_admiral()
{
	...
	ADMIN_TOOL->add_new_bid_log(
			format("New admiral %s has been created.", admiral->get_nick()));
}
...
//	SLOG("RANDOM NUMBER: %d", aNumber);
    ...
    {
		ADMIN_TOOL->add_new_bid_log(
			format("New project %s has been created.", project->get_name()));
	}
	else
	{
		ADMIN_TOOL->add_new_bid_log(
			format("Error adding project %s.", project->get_name()));
	}
}
...
CBlackMarket::create_new_planet()
{
	...
	ADMIN_TOOL->add_new_bid_log(
			format("New planet %s has been created.", planet->get_name()));
}
		...
		switch (aBid->get_type())
		{
			case CBid::ITEM_TECH:
			{
				...
				ADMIN_TOOL->add_winning_bid_log(
						format("The player %s has won the bid for the tech. %s with %d PP.",
										winner->get_nick(),
										tech->get_name_with_level(),
										aBid->get_price()));
			...
			case CBid::ITEM_FLEET:
			{
				...
				*STORE_CENTER << *admiral;
				...
				ADMIN_TOOL->add_winning_bid_log(
						format("The player %s has won the bid for the fleet %s with %d PP.",
										winner->get_nick(),
										fleet->get_nick(),
										aBid->get_price()));
			...
			case CBid::ITEM_ADMIRAL:
			{
				...
				*STORE_CENTER << *admiral;
				...
				ADMIN_TOOL->add_winning_bid_log(
						format("The player %s has won the bid for the admiral %s with %d PP.",
										winner->get_nick(),
										admiral->get_nick(),
										aBid->get_price()));
			...
			case CBid::ITEM_PROJECT:
			{
				...
				ADMIN_TOOL->add_winning_bid_log(
						format("The player %s has won the bid for the project %s with %d PP.",
										winner->get_nick(),
										project->get_name(),
										aBid->get_price()));
			...
			case CBid::ITEM_PLANET:
			{
				...
				ADMIN_TOOL->add_winning_bid_log(
						format("The player %s has won the bid for the planet %s with %d PP.",
										winner->get_nick(),
										planet->get_name(),
										aBid->get_price()));
	...
	for (int i=0 ; i<mBidList->length() ; i++)
	{
		...
		switch (Bid->get_type())
		{
			...
			case CBid::ITEM_FLEET :
			{
				...
				aMySQL->query(format("SELECT * FROM fleet WHERE owner = 0 AND id = %d", Bid->get_item()));
				aMySQL->use_result();
			...
			case CBid::ITEM_ADMIRAL :
			{
				...
				aMySQL->query(format("SELECT * FROM admiral WHERE owner = 0 AND id = %d", Bid->get_item()));
				aMySQL->use_result();
			...
			case CBid::ITEM_PLANET :
			{
				...
				aMySQL->query(format("SELECT * FROM planet WHERE owner = 0 AND id = %d", Bid->get_item()));
				aMySQL->use_result();
