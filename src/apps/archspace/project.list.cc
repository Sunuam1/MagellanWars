...
bool
CProjectList::free_item(TSomething aItem)
{
	(void)aItem;
	return true;
}
...
int
CProjectList::compare(TSomething aItem1, TSomething aItem2) const
{
	CProject
		*Project1 = (CProject *)aItem1,
		*Project2 = (CProject *)aItem2;

	if (Project1->get_id() > Project1->get_id()) return 1;
	if (Project1->get_id() < Project2->get_id()) return -1;
	return 0;
}

int
CProjectList::compare_key(TSomething aItem, TConstSomething aKey) const
{
	CProject
		*Project = (CProject *)aItem;

	if (Project->get_id() > (intptr_t)aKey) return 1;
	if (Project->get_id() < (intptr_t)aKey) return -1;
	return 0;
}

int
CProjectList::add_project(CProject* aProject)
{
...
bool
CProjectList::remove_project(int aProjectID)
{
	int
...
CProject *
CProjectList::get_by_id(int aProjectID)
{
...
char *
CProjectList::selected_project_information_html(CCommandSet &aProjectSet)
{
	static CString
		Info;
	Info.clear();
...
char *
CProjectList::selected_project_effect_html(CPlayer *aPlayer, CCommandSet &aProjectSet)
{
	static CString
		Info;
	Info.clear();
