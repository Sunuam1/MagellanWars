...
CSpy::computer_virus_infiltration(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer)
{
	...
	{
		aTargetPlayer->time_news(
				format(GETTEXT("You lost %1$s by someone's spy!"),
								(char *)News));
	}
...
CSpy::steal_common_technology(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer)
{
	...
	else
	{
		...
		aTargetPlayer->time_news(
				format(GETTEXT("Your tech %1$s has been stolen by someone's spy!"),
...
CSpy::artificial_disease(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer)
{
	...
	for (int i=0 ; i<PlanetList->length() ; i++)
	{
		...
		else
		{
			...
			aTargetPlayer->time_news(
					format(GETTEXT("You has lost some people on planet %1$s by someone's spy."),
									Planet->get_name()));
		}
	}
...
CSpy::red_death(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer)
{
	...
	for (int i=0 ; i<PlanetList->length() ; i++)
	{
		...
		if (PopulationDiff == 0)
		{
			...
			{
				aTargetPlayer->time_news(
						format(GETTEXT("You didn't lose people on planet %1$s, but the planet has an obstinate microbe now."),
										Planet->get_name()));
			}
		}
		else
		{
			...
			{
				aTargetPlayer->time_news(
						format(GETTEXT("You lost %1$d people on planet %2$s by someone's spy, and the planet has an obstinate microbe now."),
										PopulationDiff, Planet->get_name()));
			}
			else
			{
				aTargetPlayer->time_news(
						format(GETTEXT("You lost %1$d people on planet %2$s by someone's spy."),
										PopulationDiff, Planet->get_name()));
			}
		}
	}
...
CSpy::strike_base(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer)
{
	...
	{
		aTargetPlayer->time_news(
				format(GETTEXT("You lost %1$s on planet %2$s by someone's spy."),
...
CSpy::emp_storm(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer)
{
	...
	aTargetPlayer->time_news(
			format(GETTEXT("Your planet %1$s is now paralyzed for %2$d turns by someone's spy."),
...
CSpy::assassination(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer)
{
	...
	aTargetPlayer->time_news(
			format(GETTEXT("Your commander %1$s was killed by someone's spy."),
							Admiral->get_nick()));
...
CSpyTable::compare_key(TSomething aItem, TConstSomething aKey) const
{
	...
	if (Spy->get_id() > (intptr_t)aKey) return 1;
	if (Spy->get_id() < (intptr_t)aKey) return -1;
	return 0;
}
