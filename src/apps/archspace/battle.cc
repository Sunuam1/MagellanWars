	...
	CFormationInfo::compare_key( TSomething aItem, TConstSomething aKey) const
{
	...
	if ( fleet->get_id() > (intptr_t)aKey ) return 1;
	if ( fleet->get_id() < (intptr_t)aKey ) return -1;
	return 0;
}
	...
	CBattleFleetList::update_fleet_after_battle(CPlayer *aEnemy, int aWarType, bool aWin)
{
	...
	for (int i=0 ; i<length() ; i++)
	{
		...
		if (OriginalOwner->get_game_id() != EMPIRE_GAME_ID)
		{
				...
				Fleet->get_status() == CBattleFleet::STATUS_ANNIHILATED )
			{
				...
				{
					mOwner->time_news(
						format(GETTEXT("You lost the fleet %1$s and commander %2$s."),
							OriginalFleet->get_nick(), Commander->get_nick()));
				}
				else if(OriginalOwner->is_dead() == false && Dead == true && OriginalOwner != mOwner)
				{
					mOwner->time_news(
						format(GETTEXT("You lost the fleet %1$s and the commander %2$s whose owner is %3$s."),
							OriginalFleet->get_nick(),
							Commander->get_name(),
							OriginalOwner->get_nick()));

					OriginalOwner->time_news(
						format(GETTEXT("You have lost fleet %1$s and commander %2$s in %3$s's battle."),
							OriginalFleet->get_nick(),
							Commander->get_name(),
							mOwner->get_nick()));
				}
				...
				{
					mOwner->time_news(
						format(GETTEXT("You lost the fleet %1$s whose owner is %2$s."),
							OriginalFleet->get_nick(),
							OriginalOwner->get_nick()));
				}
				else if(OriginalOwner->is_dead() == false && Dead == false && OriginalOwner != mOwner)
				{
					...
					OriginalOwner->time_news(
						format(GETTEXT("You have lost fleet %1$s in %2$s's battle."),
							OriginalFleet->get_nick(),
							mOwner->get_nick()));
				}
	...
	if (ScrapPP > 0)
	{
		...
		aEnemy->time_news(
			format(GETTEXT("You have earned %1$s PP salvaging enemy ships."),
				dec2unit(ScrapPP)));
		if (aEnemy->has_ability(ABILITY_TEMPORAL_MASTERY)) {
		    aEnemy->change_ship_production(ScrapPP/5);
		    aEnemy->time_news(
			format(GETTEXT("%1$s PP has been put into the ship production pool."),
				dec2unit(ScrapPP/10)));
		}
	...
	if (mDefender->get_game_id() != 0)
	{
		...
		for (int i=0 ; i<max ; i++)
		{
			...
			if (!AllyList.get_by_game_id(Ally->get_game_id()))
			{
				...
				mRecord->add_buf(format("ALLIANCE/%d/%s/%d/%d\n",
	...
	CBattle::raid_bomb()
{
	...
	if (MilitaryBase > 0)
	{
		if (Damage*2 > MilitaryBase)
		{
			...
			mReport.format(GETTEXT("%1$s destroyed all military bases on %2$s."),
				...
				Planet->get_name());
			mReport += "<BR>\n";
			mDefender->time_news(format("%s%s.<BR>\n",
	...
	if (Damage > 0)
	{
		...
		if (Population > 0)
		{
			...
			mDefender->time_news(
				format(GETTEXT("The population was reduced by %1$s on %2$s due to enemy raids."),
	...
	if (CMission::mAdmiralExpRaid > 0)
	{
		...
		mAttacker->time_news(format(GETTEXT("%s made successful raid, he/she gained %d points of exp."),
				Admiral->get_name(),
				RealDamage));
	}
