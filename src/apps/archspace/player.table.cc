...
CPlayerTable::CPortalIDHashList::compare_key(TSomething aItem, TConstSomething aKey) const
{
	...
	if (Player->get_portal_id() > (intptr_t)aKey) return 1;
	if (Player->get_portal_id() < (intptr_t)aKey) return -1;
	return 0;
}
...
//	SLOG("compare key player %s, %d", Player->get_nick(), (intptr_t)aKey);

	if (Player->get_game_id() < (intptr_t)aKey) return -1;
	if (Player->get_game_id() > (intptr_t)aKey) return 1;
	return 0;
}
...
CPlayerTable::add_player(CPlayer* aPlayer)
{
	...
	{
		Sum += (uintptr_t)(playername[i]);
		if ((((uintptr_t)playername[i]) > 96)&&(((uintptr_t)playername[i]) < 123))
		{
			Sum -= 32;
		}
	}
...
CPlayerTable::get_by_name(const char* aName)
{
	...
	for (int i=0 ; Name[i] ; i++) {
		Sum += (uintptr_t)(Name[i]);
		if ((((uintptr_t)Name[i]) > 96)&&(((uintptr_t)Name[i]) < 123))
		{
			Sum -= 32;
		}
	}
...
CPlayerTable::verify_player()
{
	...
	for (int i=length()-1 ; i>=0 ; i--)
	{
		...
		{
			Player->set_dead(
					format("Player %1$s has been set dead by CPlayerTable::verify_player().", Player->get_nick()));
		}
