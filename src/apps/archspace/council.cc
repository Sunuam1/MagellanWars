	...
	CCouncil::set_speaker(int aGameID)
{
	...
	if (mSpeakerID == 0)
	{
		ADMIN_TOOL->add_speaker_log(
			format("The council %s has no speaker now.", (char *)mNick));
	}
	else
	{
		CPlayer *
		...
		ADMIN_TOOL->add_speaker_log(
			format("Player %s has become a speaker in the council %s.",
	...
	CCouncil::set_secondary_speaker(int aGameID)
{
	...
	else
	{
		...
		ADMIN_TOOL->add_speaker_log(
			format("Player %s has become a secondary speaker in the council %s.",
	...
	CCouncil::check_council_message(CCouncilMessage::EType aType, CCouncil* aCouncil, CCouncil *aActiveCouncil)
{
	...
	{ static CString tmp; tmp.format(GETTEXT("The message type(%1$s) is not available."),
	...
	CCouncil::break_ally(CCouncil* aCouncil)
{
	...
	send_time_news_to_all_member((char*)format(
			...
			aCouncil->get_nick()));
	aCouncil->send_time_news_to_all_member(
		format(GETTEXT("%1$s council has broken the alliance."),
			get_nick()));
	...
	*Council2 = Relation->get_council2();
		...
		aType == CCouncilMessage::TYPE_SUGGEST_ALLY)
	{
		...
		Council1->send_time_news_to_all_member(
			format(GETTEXT("Your council relationship with %1$s council has become \"%2$s\"."),
				Relation->get_council2()->get_nick(),
		...
		ADMIN_TOOL->add_council_relation_log(
			format("The relationship between the council %s and the council %s has changed into ally from peace.",
				Council1->get_nick(),
				Council2->get_nick()));
		...
		aType == CCouncilMessage::TYPE_SUGGEST_PACT)
	{
		...
		ADMIN_TOOL->add_council_relation_log(
			format("The relationship between the council %s and the council %s has changed into peace from truce.",
				Council1->get_nick(),
				Council2->get_nick()));
			...
			aType == CCouncilMessage::TYPE_DEMAND_TRUCE))
	{
		if (aType == CCouncilMessage::TYPE_DEMAND_TRUCE)
		{
			...
			{
				aCouncil->send_time_news_to_all_member(
					format(GETTEXT("Your council received the compensation(%1$s PP) for the war with %2$s council."),
						dec2unit(Compensation), get_nick()));

				send_time_news_to_all_member(
					format(GETTEXT("Your council give the compensation for war(%1$sPP) to %2$s council."),
			...
			{
				aCouncil->send_time_news_to_all_member(
					format(GETTEXT("Your council received the compensation(%1$s PP) for the total war with %2$s council."),
						dec2unit(Compensation), get_nick()));

				send_time_news_to_all_member(
					format(GETTEXT("Your council give the compensation for total war(%1$sPP) to %2$s council."),
						dec2unit(Compensation),
						aCouncil->get_nick()));
			}
		}
		...
		ADMIN_TOOL->add_council_relation_log(
			format("The relationship between the council %s and the council %s has changed into truce from war/total war.",
				Council1->get_nick(),
				Council2->get_nick()));
		...
		(aType == CCouncilMessage::TYPE_DEMAND_SUBMISSION))
	{
		if (mSupremacyCouncil == NULL)
		{
			...
			ADMIN_TOOL->add_council_relation_log(
				format("The council %s has conquered the council %s.",
					Council1->get_nick(),
					Council2->get_nick()));
		...
		else
		{
			mSupremacyCouncil->send_time_news_to_all_member(
				format(GETTEXT("%1$s has bulldozed your ordinary council %2$s."),
	...
	CCouncil::process_merge_with(CCouncil *aCouncil, CCouncil *aActiveCouncil)
{
	...
	*TargetMergingOffer = (CCouncilActionMergingOffer *)COUNCIL_ACTION_TABLE->get_by_owner_type(aCouncil, CAction::ACTION_COUNCIL_MERGING_OFFER);
	...
	send_time_news_to_all_member(
		format(GETTEXT("%1$s PP has been moved into your council's safe from the council %2$s while the councils were being merged."),
	...
	if (NumberOfTargetCouncilMember > 1)
	{
		for (int i=0 ; i<COUNCIL_TABLE->length() ; i++)
		{
			...
			Speaker->time_news(
				format(GETTEXT("The council %1$s has joined the council %2$s, so the council %3$s became stronger."),
					(char *)TargetCouncilNick,
					(char *)mNick,
					(char *)mNick));
		}
	...
	CCouncil::can_merge_with(CCouncil *aCouncil, CCouncil *aActiveCouncil)
{
	...
	if (Speaker == NULL)
	{
		{ static CString tmp; tmp.format(GETTEXT("The council %1$s has no speaker."),
			(char *)mNick);
	}
...
#define GALATIC_ECONOMICS	1112
	...
	{
		{ static CString tmp; tmp.format(GETTEXT("The speaker of the council %1$s doesn't have the tech %2$s."),
...
#undef GALATIC_ECONOMICS
	...
	if (Relationship != NULL)
	{
		...
		{
			{ static CString tmp; tmp.format(GETTEXT("The council %1$s is at war with the council %2$s."),
...
//	    SLOG("mMergePenaltyTime = %d", mMergePenaltyTime);
		...
		{
			{ static CString tmp; tmp.format(GETTEXT("The council %1$s currently cannot merge."),
				(char *)mNick);
		}
		else
		{
...
//      	        SLOG("HOURS: %d MINUTES: %d", HoursLeft, MinutesLeft);
			{ static CString tmp; tmp.format(GETTEXT("The council %1$s currently cannot merge for another %2$s."),
...
//      	        SLOG("HOURS: %d MINUTES: %d", HoursLeft, MinutesLeft);
	...
	{
		{ static CString tmp; tmp.format(GETTEXT("The council %1$s doesn't have enough space to admit the members in the council %2$s into."),
			aCouncil->get_nick(), (char *)mNick);
	}
	...
	{
		{ static CString tmp; tmp.format(GETTEXT("Your council shares no clusters with the council %1$s."),
			aCouncil->get_nick());
	}
	...
	*SmallCouncilMergingOffer = (CCouncilActionMergingOffer *)COUNCIL_ACTION_TABLE->get_by_owner_type(aCouncil, CAction::ACTION_COUNCIL_MERGING_OFFER);
	if (BigCouncilMergingOffer != NULL)
	{
		...
		if (TargetCouncil == NULL)
		{
			...
			{
				{ static CString tmp; tmp.format(GETTEXT("Your council sent merge offer to a certain council or has merge offer from it, but it doesn't exist anymore.")); return (char *)tmp; }
			}
			else
			{
				{ static CString tmp; tmp.format(GETTEXT("The council %1$s sent merge offer to a certain council or has merge offer from it, but it doesn't exist anymore."), (char *)mNick); return (char *)tmp; }
			}
		}
		else
		{
			if (TargetCouncilID != aCouncil->get_id())
			{
				if (aActiveCouncil->get_id() == mID)
				{
					{ static CString tmp; tmp.format(GETTEXT("Your council suggested the council %1$s to merge the 2 councils or has merge offer from it already."), TargetCouncil->get_nick()); return (char *)tmp; }
				}
				else
				{
					{ static CString tmp; tmp.format(GETTEXT("The council %1$s suggested the council %2$s to merge the 2 councils or has merge offer from it already."), (char *)mNick, TargetCouncil->get_nick()); return (char *)tmp; }
				}
			}
		}
	}
	else if (SmallCouncilMergingOffer != NULL)
	{
		...
		if (TargetCouncil == NULL)
		{
			if (aActiveCouncil->get_id() == mID)
			{
				{ static CString tmp; tmp.format(GETTEXT("The council %1$s sent merge offer to a certain council or has merge offer from it, but it doesn't exist anymore."), aCouncil->get_nick()); return (char *)tmp; }
			}
			else
			{
				{ static CString tmp; tmp.format(GETTEXT("Your council sent merge offer to a certain council or has merge offer from it, but it doesn't exist anymore.")); return (char *)tmp; }
		...
		else
		{
			if (TargetCouncilID != mID)
			{
				...
				{
					{ static CString tmp; tmp.format(GETTEXT("The council %1$s suggested the council %2$s to merge the 2 councils or has merge offer from it already."), aCouncil->get_nick(), TargetCouncil->get_nick()); return (char *)tmp; }
				}
			}
		}
	}
